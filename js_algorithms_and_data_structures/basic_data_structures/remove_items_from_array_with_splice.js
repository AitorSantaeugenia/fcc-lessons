...
in the middle? Or remove more than one element at once? Well, that's where splice()
 comes in. splice() allows us to do just that: remove any number of consecutive elements 
...
splice() can take up to 3 parameters, but for now, we'll focus on just the first 2.
 ...
 so to indicate the first element of an array, we would use 0. splice()'s first parameter
  represents the index on the array from which to begin removing elements, while the 
  second parameter indicates the number of elements to delete. For example:

let array = ['today', 'was', 'not', 'so', 'great'];

array.splice(2, 2);
Here we remove 2 elements, beginning with the third element (at index 2). array would
 have the value ['today', 'was', 'great'].
...
//Done 4fun with removing characters from the array
const arr = [ 2, 4, 5, 1, 7, 5, 2, 1 ];
// Only change code below this line
let test = arr.splice(1, 3);
// Only change code above this line
console.log(test);
